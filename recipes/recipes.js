...
function search() {
    ...
    // Clear out any previous content entirely
...
// Target DOM nodes securely by matching IDs
...
/* Activates search when hitting the Enter key */
...
function tagTemplate(tags) {
    return tags.map((tag)=> `<button class="category-tag">${tag}</button>`).join(' ');
}
...
function recipesTemplate(recipe) {
    ...
    <div class="recipe-card">
        <div class="recipe-image-wrapper">
            <img class="apple-crisp-img" src="${recipe.image}" alt="${recipe.name}">
        </div>
